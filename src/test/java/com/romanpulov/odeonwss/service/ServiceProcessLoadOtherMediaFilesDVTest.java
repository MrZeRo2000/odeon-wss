...
public class ServiceProcessLoadOtherMediaFilesDVTest {
    private static final Logger log = Logger.getLogger(ServiceProcessLoadOtherMediaFilesDVTest.class.getSimpleName());

    @Value("${test.data.path}")
    String testDataPath;

    private enum TestFolder {
        TF_SERVICE_PROCESS_LOAD_OTHER_MEDIA_FILES_DV_TEST_OK
    }
    ...
    public void setup() throws Exception {
        ...
        FileTreeGenerator.generateFromJSON(
                tempFolders.get(TestFolder.TF_SERVICE_PROCESS_LOAD_OTHER_MEDIA_FILES_DV_TEST_OK),
                this.testDataPath,
                """
                        {
    ...
    void testPrepare() {
        service.executeProcessor(
                ...
                tempFolders.get(TestFolder.TF_SERVICE_PROCESS_LOAD_OTHER_MEDIA_FILES_DV_TEST_OK).toString());
    ...
    void testLoadMediaFiles() {
        ...
        service.executeProcessor(
                ProcessorType.DV_OTHER_MEDIA_LOADER,
                tempFolders.get(TestFolder.TF_SERVICE_PROCESS_LOAD_OTHER_MEDIA_FILES_DV_TEST_OK).toString());

        assertThat(service.getProcessInfo().getProcessingStatus()).isEqualTo(ProcessingStatus.SUCCESS);
        log.info("Other Media Loader Processing info: " + service.getProcessInfo());
