...
public class ServiceProcessLoadOtherDVTest {
    private final static String[] DV_PRODUCT_NAMES = {
            "Genesis - Together and Apart",
            "Michael Flatley - Lord of the Dance"
    };

    private static final Logger log = Logger.getLogger(ServiceProcessLoadOtherDVTest.class.getSimpleName());

    @Value("${test.data.path}")
    String testDataPath;

    private enum TestFolder {
        TF_SERVICE_PROCESS_LOAD_OTHER_DV_TEST_OK
    }
    ...
    public void setup() throws Exception {
        ...
        FileTreeGenerator.generateFromJSON(
                tempFolders.get(TestFolder.TF_SERVICE_PROCESS_LOAD_OTHER_DV_TEST_OK),
                this.testDataPath,
                """
{
  "Genesis - Together and Apart": {
  ...
  "Michael Flatley - Lord of the Dance": {
    ...
    private static final ProcessorType PROCESSOR_TYPE = ProcessorType.DV_OTHER_LOADER;
    ...
    void testSuccess() throws Exception {
        processService.executeProcessor(
                PROCESSOR_TYPE,
                tempFolders.get(TestFolder.TF_SERVICE_PROCESS_LOAD_OTHER_DV_TEST_OK).toString());
        ProcessInfo pi = processService.getProcessInfo();
        List<ProcessDetail> processDetails = pi.getProcessDetails();
        log.info("Other Loader Processing info: " + processService.getProcessInfo());
        ...
        assertThat(dto.getProcessDetails().get(1).getRows()).isEqualTo(2L);
    ...
    void testTrackNumber() {
        ...
        var track = trackRepository.findTrackByArtifactAndTitle(artifact, "Genesis - Together and Apart").orElseThrow();
